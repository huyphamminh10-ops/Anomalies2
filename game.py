...
class AnomalyChatManager:
    ...
    # ── Tạo kênh ──────────────────────────────────────────────────────────────
    async def create(self, guild: discord.Guild, category=None):
        try:
            overwrites = {
            ...
            self.logger.info(f"Anomaly Chat created: #{self.channel.name}")
        except Exception as e:
            self.logger.error(f"Failed to create Anomaly Chat: {e}")

    # ── Cấp quyền cho 1 thành viên (đọc + ghi) ────────────────────────────────
    async def add_member(self, member, send: bool = True):
        """Cho phép một thành viên đọc/ghi kênh."""
        if not self.channel:
            return
        try:
...
class DeadChatManager:
    ...
    async def create(self, guild: discord.Guild, category: Optional[discord.CategoryChannel] = None):
        """Create the Dead Chat channel with restricted permissions."""
        try:
            overwrites = {
                guild.default_role: discord.PermissionOverwrite(
                    read_messages=False,
            ...
            self.logger.info(f"Dead Chat created: #{self.channel.name}")
        except Exception as e:
            self.logger.error(f"Failed to create Dead Chat: {e}")
...
class GameEngine:
    ...
    async def start(self, survivor_classes, anomaly_classes, unknown_classes):
        try:
            ...
            # Create dead chat
            await self.dead_chat_mgr.create(self.guild)
            self.dead_chat = self.dead_chat_mgr.channel

            # Create anomaly chat
            await self.anomaly_chat_mgr.create(self.guild)
            ...
            try:
                ...
                # Xóa tất cả tin nhắn, chỉ giữ thông báo lỗi
    ...
    async def _purge_text_channel(self, keep_id: int = None, reason: str = "Tự Động Dọn Dẹp"):
        """
        Xóa tất cả tin nhắn trong text_channel trừ keep_id.
        Gửi thông báo số tin đã xóa, tự xóa sau 15 giây.
        """
        if not self.text_channel:
            return
        try:
            def should_delete(msg):
                return msg.id != keep_id if keep_id else True

            deleted = []
            try:
                deleted = await self.text_channel.purge(limit=200, check=should_delete, bulk=True)
            except discord.Forbidden:
                async for msg in self.text_channel.history(limit=200):
    ...
    async def end_game(self, winner: str):
        ...
        # Xóa toàn bộ tin nhắn trong kênh, chỉ giữ bảng kết quả
