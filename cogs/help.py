...
GAMEPLAY_FAQ = [
    ...
    },
    {
        ...
        "a": (
            "**Ghi di chúc:**\n"
            "Gõ `Nhập di chúc` trong kênh chat của game.\n"
            "Bot sẽ hướng dẫn — mỗi tin nhắn tiếp theo là **1 dòng** di chúc.\n\n"
            "• Tối đa **45 dòng**, mỗi dòng tối đa **60 ký tự** (không tính dấu cách).\n"
            "• Dòng hợp lệ → bot react ✅ + số thứ tự.\n"
            "• Dòng quá dài → bot react 🚫 + số thứ tự, kèm thông báo.\n"
            "• Di chúc được lưu tự động sau mỗi dòng — không cần lệnh kết thúc.\n"
            "• Khi bạn **chết**, di chúc tự động khóa lại.\n\n"
            "**Xem di chúc:**\n"
            "Mỗi sáng, bot gửi bảng **📜 LÁ THƯ NGƯỜI CHẾT** kèm menu chọn.\n"
            "Chọn tên người chết → bot gửi **DM** file `.txt` chứa toàn bộ di chúc."
        ),
    },
    {
        "emoji": "💬",
