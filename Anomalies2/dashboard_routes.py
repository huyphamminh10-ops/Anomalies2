...
async def serve_dashboard(request: Request, full_path: str = ""):
    """Serve toàn bộ Dashboard SPA từ file index.html."""
    html_path = os.path.join(os.path.dirname(__file__), "dashboard", "index.html")
    if not os.path.exists(html_path):
        return HTMLResponse(
            "<h2 style='font-family:sans-serif;padding:40px'>Dashboard chưa được deploy.<br/>"
            "Hãy đặt file <code>dashboard/index.html</code> cạnh <code>app.py</code>.</h2>",
            status_code=503,
        )
