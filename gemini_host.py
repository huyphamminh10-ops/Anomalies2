"""
Gemini Host (Quản Trò AI) — dùng aiohttp gọi REST API trực tiếp,
KHÔNG dùng google-genai SDK để tránh conflict websockets trên HF Spaces.
...
class GeminiHost:
    ...
    async def on_day_start(self, day_count: int = 0):
        ...
        if self.logger:
            self.logger.info(f"GeminiHost: Day {day_count} — MAIN ON, ANOMALIES OFF.")
    ...
    async def on_day_end(self):
        ...
        if self.logger:
            self.logger.info("GeminiHost: Night — DEAD ON, ANOMALIES ON, MAIN OFF.")
    ...
    # ============================================================
    #  Gọi Gemini qua REST API trực tiếp (không dùng SDK)
    # ============================================================
    ...
    async def _generate(
        ...
        url = f"{_GEMINI_API_BASE}/{_MODEL_NAME}:generateContent?key={self._api_key}"
        payload = {
            "system_instruction": {
        ...
        try:
            ...
            async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                if resp.status != 200:
                    ...
                    if self.logger:
                        self.logger.warn(
                            f"GeminiHost[{_LABELS[key]}]: API HTTP {resp.status}: {body[:200]}"
                        )
                    return None
                data = await resp.json()
        except Exception as e:
            if self.logger:
                self.logger.warn(f"GeminiHost[{_LABELS[key]}]: request lỗi: {e}")
            return None
        ...
        try:
            ...
            if self.logger:
                self.logger.warn(f"GeminiHost[{_LABELS[key]}]: parse response lỗi: {e} | {data}")
            return None
